...
type CardType = '시스템 알림' | '일기 알림';

interface NotificationTabProps {
  onChange?: (selected: CardType) => void; // 선택값을 반환
}

const NotificationTab: React.FC<NotificationTabProps> = ({ onChange }) => {
  const [selected, setSelected] = useState<CardType>('시스템 알림');
  const tabRefs = useRef<HTMLButtonElement[]>([]);
  const [underlineStyle, setUnderlineStyle] = useState<{left: number, width: number}>({left: 0, width: 0});

  const tabItemList: CardType[] = ["시스템 알림", "일기 알림"];
  ...
  return (
    <div className="mt-3 relative flex border-b-[3px] border-[#F2F2F2] w-full mx-auto">
